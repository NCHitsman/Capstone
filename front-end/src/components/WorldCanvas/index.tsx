      ...
      {world && settlements ? (
        <div>
          ...
          >
            <axesHelper />
            <gridHelper
              args={[world?.world_size, world?.world_size]}
              position={[0, 0, 0]}
            />
            <OrbitControls />
            <ambientLight />
          ...
          <div
            className={
              hidden
                ? "settlement__cards__parent none"
                : "settlement__cards__parent"
            }
          >
            <div hidden={hidden}>Name: {settlements[selectedId]?.name}</div>
            <div hidden={hidden}>Type: {settlements[selectedId]?.type}</div>
            <div hidden={hidden}>
              Population: {settlements[selectedId]?.population}
            </div>
            <div hidden={hidden}>Wealth: {settlements[selectedId]?.wealth}</div>
            <button
              onClick={() => {
